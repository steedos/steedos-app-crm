Creator.Objects['account'].triggers = {
    ...
    "after.update.server.disableSpaceUsers": {
        ...
        todo: function (userId, doc, fieldNames, modifier, options) {
            ...
            if (modifier.$set.is_supplier === false) {
                var contacts = Creator.getCollection("contact").find({account: doc._id, space: doc.space, user: {$exists: true}}, {fields: {_id:1}}).fetch();
                ...
                Creator.getCollection("contact").direct.update({_id:{$in: supplierContactIds}}, {$unset: {user: 1}}, {
                    multi: true
                });
            ...
            if (modifier.$set.is_customer === false) {
                ...
                Creator.getCollection("contact").direct.update({_id:{$in: customerContactIds}}, {$unset: {user: 1}}, {
                    multi: true
                });
